...
def fetch_and_save_nav_csv():
    ...
    try:
        ...
        # First pass: find header and count total schemes
        for row in all_rows:
            row_count += 1
            if not header:
                header = row
                ...
                if len(row) >= len(col_indices) and row[0].strip().isdigit():
                    total_schemes += 1
        ...
        # Second pass: process rows with progress bar
        with tqdm(desc="Processing schemes", unit="schemes", total=total_schemes, disable=False) as pbar:
            for row in all_rows:
                if row == header:
        ...
        print(f"Total rows processed: {row_count}. Cleaned rows: {len(cleaned_rows)}")

        print("Sorting data rows by SchemeCode...")
