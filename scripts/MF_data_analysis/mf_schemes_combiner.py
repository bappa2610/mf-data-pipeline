...
def combine_schemes():
    print("Copying amfi_nav_all.csv to analytics folder as base file...")
    ...
    try:
        ...
        print(f"Base file created with {len(df_base)} rows from amfi_nav_all.csv")
    ...
    print("Reading mfapi_schemes_matched.csv...")
    try:
        df_mfapi = pd.read_csv(mfapi_matched_file)
        df_mfapi = df_mfapi[required_columns]
        print(f"Loaded {len(df_mfapi)} rows from mfapi_schemes_matched.csv")
    except FileNotFoundError:
    ...
    print("Reading modified_portal.amfi.csv_schemes.csv...")
    try:
        df_portal = pd.read_csv(portal_modified_file)
        df_portal = df_portal[required_columns]
        print(f"Loaded {len(df_portal)} rows from modified_portal.amfi.csv_schemes.csv")
    except FileNotFoundError:
    ...
    print("Removing duplicates based on SchemeCode...")
    ...
    print(f"After deduplication, {len(df_combined)} unique rows")

    print("Saving to base file...")
    # Save to the base file (overwriting it)
    df_combined.to_csv(base_file, index=False)
    print(f"Successfully saved {len(df_combined)} rows to {base_file}")
